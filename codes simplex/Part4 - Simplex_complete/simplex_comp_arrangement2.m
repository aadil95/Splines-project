...
title('simplex covering the entire data region (arrangement2)')
...
title(['plot B coefficient location in simplex tj:' num2str(tj) '(arrangement2)'])
end
...
xticks(x_plot);legend;grid on;title('RMS relative error with fixed continuity order:0 (arrangement2)');xlabel('model degree');ylabel('Rel RMS error')
%% Mean Parameter Variance as degree increases and continuity fixed
figure
x_plot=1:10';
y_plot=var_param_log(1:10);
plot(x_plot,y_plot)
xticks(x_plot);grid on;title('Mean Parameter Variance with fixed continuity order:0 (arrangement2)');xlabel('model degree');ylabel('log10 $$ (\overline {Var(parameters )} ) $$','Interpreter', 'Latex')
%% plot validation error of order 3 and 9
val_degree1=3;
val_degree2=9;
figure;
subplot(1,2,1)
plot(residual_validation{val_degree1});title(['validation Model residual of degree:' num2str(val_degree1) ' continuity order:0 (arrangement2)']);grid on;xlabel('samples');ylabel('error')
subplot(1,2,2)
plot(residual_validation{val_degree2});title(['validation Model residual of degree:' num2str(val_degree2) ' continuity order:0 (arrangement2)']);grid on;xlabel('samples');ylabel('error')
...
plot(x_plot,y_plot_tr,'DisplayName','training');
plot(x_plot,y_plot_val,'DisplayName','validation');
xticks(x_plot);legend;grid on;title('RMS relative error with fixed degree: 4 (arrangement2)');xlabel('continuity order');ylabel('Rel RMS error')
